...
class SHA256transition( object ):

    def __init__( self, message ):
        self.bv = BitVector(textstring = message)

    def sha256transition(self):

        #  8 64-bit vals to initialize regs
...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 3:
        sys.stderr.write("req 3 args")
        sys.exit(1)
    
    inputName = sys.argv[1]
    outputName = sys.argv[2]
    with open(inputName, 'r') as file_to_hash:
        messageRead = file_to_hash.read()
    hasher = SHA256transition( message = messageRead )
    hashval_in_hex = hasher.sha256transition()

    with open(outputName, "w") as outputFile:
        outputFile.write(hashval_in_hex)
