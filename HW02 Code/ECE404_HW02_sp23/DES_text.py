...
def encrypt(plainText, encrypt_keys):

    pt_bv = BitVector(textstring = plainText)
    ...
    cipherText = "" #text to write to encryptOutputFile
    ...
    while (encrypted < blockCount):
        ...
        cipherText += currentBlock.get_bitvector_in_hex()
        encrypted += 1
    
    return cipherText


def decrypt(plainText, decrypt_keys):
    pt_bv = BitVector(hexstring = plainText)

    #prepare for loop
    blockCount = pt_bv.size // 64 #total blocks to decrypt
    decrypted = 0 #number of blocks thus far decrypted
    plainText = "" #text to write to decryptOutputFile

    while (decrypted < blockCount):
        bitStart = decrypted * 64
        currentBlock = pt_bv[bitStart:(bitStart + 64)]
        for roundNo in range(0, 16):
            (finalLE, finalRE) = encRound(currentBlock, decrypt_keys[roundNo])
            
            if(roundNo == 15):
                currentBlock = finalRE.__add__(finalLE)
            else:
                currentBlock = finalLE.__add__(finalRE)
        plainText += currentBlock.get_bitvector_in_ascii()
        decrypted += 1
...
#preprocessing 
if len(sys.argv) != 5:                                                  
    sys.exit('''Needs four command-line arguments, a tag, I/O, and a key''')
...
if sys.argv[1] == "-e":
    outputText = encrypt(inputText, round_keys)
elif sys.argv[1] == "-d":
    outputText = decrypt(inputText, round_keys[::-1])
else:
    sys.exit('''bad flag''')
