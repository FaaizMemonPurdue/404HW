...
def encrypt(plainName, encryptionKey, encryptOutputFile):
    key = keytext_to_deskey(open(encryptionKey).read())
    round_keys = generate_round_keys(key)
    
    # round_key = extract_round_key( key )
    plainText = open(plainName).read()
    bv = BitVector(textstring = plainText)
    padNo = 64 - (bv.size % 64)
    padder = BitVector(bitlist = [0] * padNo) # you know what this does
    bv = bv.__add__(padder)
    blockCount = bv.size // 64
    encrypted = 0
    while (encrypted < blockCount):
        bitStart = encrypted * 64
        currentBlock = bv[bitStart:(bitStart + 64)]
        for roundNo in range(0, 16):
            
            [LE, RE] = currentBlock.divide_into_two()
            
            newRE = RE.permute( expansion_permutation ) #new RE is now 48 bits (implicitly split/append)
            #E-step complete
            round_key = round_keys[roundNo] #permutation/contraction to  48 bit roundKey
            out_xor = newRE ^ round_key
            out_xor = substitute(out_xor) # S box step, now 32 bits
            out_xor = out_xor.permute(p_box)
            finalRE = out_xor ^ LE
            finalLE = RE
            if(roundNo == 15):
...
if __name__ == "__main__":
    print("Hello, World!")
    encrypt("message.txt", "key.txt", "encryptedMessage.txt")